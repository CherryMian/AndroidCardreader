...
public final class TrackUtils {
	...
	public static boolean extractTrack2Data(final EmvCard pEmvCard, final byte[] pData) {
		...
		if (!ret) {
			byte[] pan = TlvUtil.getValue(pData, EmvTags.PAN);
			if (pan != null) {
				String panStr = BytesUtils.bytesToStringNoSpace(pan);
				// Remove all trailing 'F' padding
				panStr = panStr.replaceAll("(?i)[F]+$", "");
				pEmvCard.setCardNumber(panStr);
				ret = true;

				byte[] expiry = TlvUtil.getValue(pData, EmvTags.APP_EXPIRATION_DATE);
				if (expiry != null) {
					String expiryStr = BytesUtils.bytesToStringNoSpace(expiry);
					// Ensure we interpret the first 4 digits as YYMM
					if (expiryStr.length() > 4) {
						expiryStr = expiryStr.substring(0, 4);
					}
					SimpleDateFormat sdf = new SimpleDateFormat("yyMM", Locale.US);
					try {
						pEmvCard.setExpireDate(CommonsUtils.truncate(sdf.parse(expiryStr), Calendar.MONTH));
					} catch (ParseException e) {
						LOGGER.error("Unparsable expire card date from 5F24 : {}", e);
					}
				}
			}
		}
